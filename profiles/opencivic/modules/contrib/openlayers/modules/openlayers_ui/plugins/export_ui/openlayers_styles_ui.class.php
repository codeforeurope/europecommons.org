...
class openlayers_styles_ui extends ctools_export_ui {
  ...
  function edit_form(&$form, &$form_state) {
    ...
    foreach ($properties as $key => $prop) {
      $form['data'][$key] = array(
        '#datatype' => !isset($prop['type']) ? 'text' : $prop['type'],
  ...
  function edit_form_submit(&$form, &$form_state) {
    $style_data = $form_state['complete form']['data'];
    ...
    foreach ($form_state['values']['data'] as $key => $value) {
      ...
      elseif (isset($style_data[$key]['#datatype']) &&
        strpos($form_state['values']['data'][$key], '${') !== 0) {
        if ($style_data[$key]['#datatype'] == 'integer') {
          $form_state['values']['data'][$key] =
            (int) $form_state['values']['data'][$key];
        }
        elseif ($style_data[$key]['#datatype'] == 'float') {
          $form_state['values']['data'][$key] =
